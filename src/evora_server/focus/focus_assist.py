...
def extract_source(data, max_sources=50):
    ...
    sources = sep.extract(
        ...
        1.5,
        err=bkg.globalrms,
        minarea=config.FOCUS_SEP_MIN_AREA,
    )
...
def calc_hfd(signal, aperture):
    try:
        ...
        dist_pix_pairs = []
        for (y, x), pix in numpy.ndenumerate(roi_data):
            dx = x - aperture.r
            dy = y - aperture.r
            dist = numpy.sqrt(dy * dy + dx * dx)
            if dist < config.FOCUS_APERTURE_R:
                dist_weighted_flux += pix * dist
                dist_pix_pairs.append((dist, pix))

        # total_flux = aperture_photometry(signal, aperture)['aperture_sum'][0]
        total_flux = numpy.sum(roi_data)
...
def calc_fwhm(signal, aperture):
    ...
    edge_radii = numpy.arange(25)
    rp = RadialProfile(signal, xycen, edge_radii, mask=None)
    fwhm_value = rp.gaussian_fwhm
    return fwhm_value


def stat_for_image(fits_file_url: str) -> tuple[float, float, float, float]:
    ...
    # SEP HFD
    hfrs, flag = sep.flux_radius(
        signal, sources["x"], sources["y"], 6.0 * sources["a"], frac=0.5, subpix=5
    )
    median_sep_hfd = numpy.median(hfrs[flag == 0]) * 2
    ...
    for x, y in zip(x_coords, y_coords):
        aperture = CircularAperture((x, y), r=config.FOCUS_APERTURE_R)
    ...
    median_fwhm = numpy.median(fwhm_values)
    median_my_hfd = numpy.median(my_hfd_values)
    median_phd_hfd = numpy.median(phd_hfd_values)

    return median_fwhm, median_sep_hfd, median_my_hfd, median_phd_hfd


def find_focus_position(
    focuser_positions: list[float],
    fwhm_curve_dp: list[float],
    ...
    fwhm_fit = list(numpy.polyfit(focuser_positions, fwhm_curve_dp, 2))
    fwhm_min_value = float(-fwhm_fit[1] / (2 * fwhm_fit[0]))

    logger.info(f"FWHM predicted focuser position is {fwhm_min_value:.2f}")

    hfd_fits = {}
    hfd_min_values = {}
    for method, hfd_curve_dp in hfd_curve_dps.items():
        hfd_fit = list(numpy.polyfit(focuser_positions, hfd_curve_dp, 2))
        hfd_min_value = float(-hfd_fit[1] / (2 * hfd_fit[0]))
        logger.info(f"{method} HFD predicted focuser position is {hfd_min_value:.2f}")
...
def plot_fit(
    ...
    x_fit = numpy.linspace(min(focuser_positions), max(focuser_positions), 100)

    # plot FWHM curve and fit
    ax1.plot(focuser_positions, fwhm_curve_dp, "o")
    if should_plot_fit:
        y_fit = numpy.polyval(fwhm_fit, x_fit)
        ...
        ax1.axvline(
            ...
            color="r",
            linestyle="--",
            label=f"Minimum FWHM: {fwhm_min_value:.2f}",
        )
    ...
    for method, hfd_curve_dp in hfd_curve_dps.items():
        ...
        if should_plot_fit:
            ...
            ax2.axvline(
                ...
                color=c,
                linestyle="--",
                label=f"Minimum {method} HFD: {hfd_min_value:.2f}",
            )
