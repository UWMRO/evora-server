                ...
                if len(data_str.split(" ")) >= 2:
                    try:
                        if int(data_str.split(" ")[1]) not in range(0, 6):
                            writer.write(
                                b"""Error: Invalid position number.
                        ...
                        message = (
                            f"Success! Moved to filter position {data_str.split()[1]}\n"
                        )
                        writer.write(message.encode("utf-8"))
                    except ValueError:
                        writer.write(
                            b"""Error: Invalid position character. 
                                        Position character must be a number range from 0 to 5.\n"""
                        )
                else:
                    writer.write(
                        b"Error: Unknown error occurred while moving filter wheel.\n"
                    )
